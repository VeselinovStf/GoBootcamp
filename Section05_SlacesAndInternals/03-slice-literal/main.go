...
func main() {
	...
	names = []string{"Jon", "Tom", "Ivan"}
	distances = []int{4, 5, 6, 7, 5}
	data = []byte{12, 23, 54, 65, 12}
	ratios = []float64{22.3, 0.213}
	alives = []bool{true, false}

	fmt.Printf("names    : %T %d %t %t\n", names, len(names), names == nil, len(names) == cap(names))
	fmt.Printf("distances: %T %d %t %t\n", distances, len(distances), distances == nil, len(distances) == cap(distances))
	fmt.Printf("data     : %T %d %t %t\n", data, len(data), data == nil, len(data) == cap(data))
	fmt.Printf("ratios   : %T %d %t %t\n", ratios, len(ratios), ratios == nil, len(ratios) == cap(ratios))
	fmt.Printf("alives   : %T %d %t %t\n", alives, len(alives), alives == nil, len(alives) == cap(alives))
